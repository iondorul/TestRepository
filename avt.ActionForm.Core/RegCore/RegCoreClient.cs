...
using avt.ActionForm.RegCore.Cryptography;
    ...
    internal class RegCoreClient : IRegCoreClient
    {
        ...
        static public RegCoreClient Get(string regCoreSrv, string productCode, IActivationDataStore src, bool clearCache)
        {
            lock (typeof(RegCoreClient)) {
                ...
                if (clearCache == false && HttpRuntime.Cache["avt.ActionForm.RegCoreClient." + productCode] != null) {
                    regCoreClient = (RegCoreClient)HttpRuntime.Cache["avt.ActionForm.RegCoreClient." + productCode];
                } else {

                    // clear cache, in case it's ignored
                    HttpRuntime.Cache.Remove("avt.ActionForm.RegCoreClient." + productCode);

                    // create new client
                    regCoreClient = new RegCoreClient(regCoreSrv, src);
                    HttpRuntime.Cache.Insert("avt.ActionForm.RegCoreClient." + productCode, regCoreClient);
                }
        ...
        public ILicenseActivation Activate(string regCode, string productCode, string version, string host, string productKey)
        {
            ...
            act.Host = xmlAct.DocumentElement["host"].InnerText;
            act.ActivationCode = xmlAct.DocumentElement["activation_code"].InnerText;
            act.ProductKey = xmlAct.DocumentElement["product_key"].InnerText;
            act.BaseProductCode = r.ProductCode;
            act.BaseProductVersion = xmlAct.DocumentElement["version"].InnerText;
